...
for i in 1:length(idx)-1   # -1 perché ho scoperto che la traiettoria che andava fuori scala erano l'ultimma, in entrambi i casi (-4 x Brownian)
...
plot!(xMSD,MSD, marker=true,legend=false);
xlabel!("Time [s]");
ylabel!("MSD [μ²]")
...
fit2=LsqFit.curve_fit(model,xMSD,MSD,p0,lower=[0.1*D,0.0],upper=[10*D,Inf])
p=fit2.param
plot!(xMSD,model(t,p), label="Fit")
...
png(graphSD, "BrownianMotion_traks")
png(graphSD, "+H2O2:_traks")
